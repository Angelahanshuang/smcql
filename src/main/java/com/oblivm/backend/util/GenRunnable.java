...
public abstract class GenRunnable<T> extends com.oblivm.backend.network.Server implements Runnable {
	...
	public int[] runCore() throws Exception {
		...
		try {
			...
			try {
				 if(verbose)
					System.out.println("connecting listen:" + String.valueOf(port));
	            listen(port);
	            if(verbose)
	            ...
	            s = System.nanoTime();
				System.out.println("[CODE]GenRunnable prepareInput");
	            prepareInput(env);
	            os.flush();
				System.out.println("[CODE]GenRunnable secureCompute");
	            secureCompute(env);
	            os.flush();
	            System.out.println("[CODE]GenRunnable prepareOutput");
